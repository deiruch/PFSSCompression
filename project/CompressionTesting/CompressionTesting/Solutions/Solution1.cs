...
namespace CompressionTesting.Solutions
{
    class Solution1 : ISolution
    {
        ...
        public TestResult Four(PFSS.PFSSData data, int qualityLevel, string folder)
        {
            ...
            //Residualizer.DoResiduals(data, 3);
            /*DebugOutput.MedianWriter.AnalyzePerCurveType(data,TYPE.OUTSIDE_TO_SUN,0,new FileInfo(Path.Combine(folder, this.GetName()+"ots.csv")));
            DebugOutput.MedianWriter.AnalyzePerCurveType(data, TYPE.SUN_TO_OUTSIDE, 0, new FileInfo(Path.Combine(folder, this.GetName() + "sto.csv")));
            DebugOutput.MedianWriter.AnalyzePerCurveType(data, TYPE.SUN_TO_SUN, 0, new FileInfo(Path.Combine(folder, this.GetName() + "sts.csv")));
            DebugOutput.MedianWriter.AnalyzeFirstCurveType(data, TYPE.OUTSIDE_TO_SUN, new FileInfo(Path.Combine(folder, this.GetName() + "ots_curve.csv")));*/
